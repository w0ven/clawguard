...
|---|---|
| 文本 | 关键词/链接/AI 审核 |
| 编辑消息 | 重新进入审核链，不重复累计毕业计数 |
| 图片 | 可走视觉 AI 审核 |
| 视频 | 可抽关键帧走视觉 AI 审核 |
| 文件 | 非文本策略 + AI 文本占位审核 |
| 联系人名片 | 提取姓名、电话、Telegram ID 审核 |
| 贴纸 | 静态贴纸走图片审核，动态/视频贴纸审核缩略图 |
| GIF/动画 | 非文本策略 + 可抽帧审核 |
| 语音 | 非文本策略 + AI 文本占位审核 |
| 音频 | 非文本策略 + AI 文本占位审核 |
| 视频消息 | 非文本策略 + 可抽帧审核 |
...
| 地点 | 提取标题、地址、经纬度审核 |
| 游戏 | 提取标题和描述审核 |
| Invoice | 提取标题、描述、总额、货币审核 |
| Story 转发/回复 | 提取来源和 story ID 审核 |
| Giveaway | 提取奖品描述、数量、截止时间审核 |
| 跨聊天引用/链接预览 | 展开引用或预览上下文后审核 |

...
export const models=[['tools',true,true],['backup',true,true],['text',false,true],['disabled',true,false]].map(([key,supports_tools,enabled],i)=>({id:i+1,provider_id:1,provider_key:'fixture',model_key:key,ref:`fixture:${key}`,label:`Fixture ${key}`,api_format:'openai_chat',enabled,supports_tools,supports_vision:false,supports_json:true,capability_tags:[],priority:i}));
...
export const test=base.extend<{api:Harness}>({api:[async({page},use,info)=>{
 ...
 await page.route('**/*',async route=>{
  ...
  if(path==='/api/auth/me')data={admin:{id:7,telegram_id:7,role:'super_admin',display_name:'Fixture 验证管理员'}};
   ...
   const overview=()=>({policy:p,model_pool:pool,defaults:{disabled:true,retention_days:7,history_retention:'7 days',remote_quota:'unknown'}});
   if(!suffix&&method==='GET')data=overview();
   else if(!suffix&&method==='PUT'){if(body.expected_version!==p.version)return reject(409,'policy version changed');api.policies[id]={...p,...body,version:p.version+1};delete api.policies[id].expected_version;data={policy:api.policies[id]};}
...
export async function tab(page:Page,name:string){await page.getByRole('button',{name,exact:true}).click();}
export async function memoryTab(page:Page){await tab(page,'记忆中心');await expect(page.getByText('Fixture -1001 事实1',{exact:true})).toBeVisible();}

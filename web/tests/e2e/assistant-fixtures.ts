...
export type Harness={writes:any[],requests:any[],errors:string[],console:any[],external:string[],unknown:string[],injected:any[],failures:any[],rules:Rule[],policies:Record<string,any>,pools:Record<string,any>,memories:Record<string,any[]>,conflicts:Record<string,any[]>,status:Record<string,any>,dispatches:Record<string,any[]>,nativeEngine:boolean,native:any,nativeGroups:Record<string,any>,nativeMemories:Record<string,any[]>};
export const test=base.extend<{api:Harness}>({api:[async({page},use,info)=>{
 let unauthorized=false;
 const api:Harness={writes:[],requests:[],errors:[],console:[],external:[],unknown:[],injected:[],failures:[],rules:[],policies:{},pools:{},memories:{},conflicts:{},status:{},dispatches:{},nativeEngine:false,native:{engine:'native',revision:1,bot:{max_context_tokens:256000,memory_recent_messages:500},prompts:{persona:'',casual:''},schema:{properties:{max_context_tokens:{type:'integer'},memory_recent_messages:{type:'integer'}}},source_unwired:{},music:{},movie_info:{},stickers:{},extra_schema:{},movie_secret_fields:[],movie_secrets_configured:{},secret_storage_ready:true},nativeGroups:{},nativeMemories:{}};
 ...
 await page.route('**/*',async route=>{
  ...
  if(path==='/api/auth/me')data={admin:{id:7,telegram_id:7,role:'super_admin',display_name:'Fixture 验证管理员'}};
   ...
   const overview=()=>{const selected=models.find(m=>m.ref===p.chat_model_ref);const declared=selected?.tool_capability==='declared';const blockers=!p.chat_model_ref?['还不能回复：请先选择聊天模型。']:!selected?['当前聊天模型不可用']:!declared?['这个模型还没声明能调用技能，请去模型管理打开「支持工具调用」声明，或换一个已声明的模型。']:[];return {native_engine:!!api.nativeEngine,policy:p,model_pool:pool,defaults:{disabled:true,retention_days:7,history_retention:'7 days',remote_quota:'unknown'},readiness:{can_chat:Boolean(p.chat_enabled&&declared),blockers,chat_primary_model_ref:p.chat_model_ref||'',tools_declared:declared}};};
   if(suffix.startsWith('/native')){
    api.nativeGroups[id]??={settings:{cg_config_revision:0,at_reply_mode:false,tts_mode:'off',mute_all_replies:false,scheduled_tasks:{cooldown_topic:{enabled:false,task_brief:''}},speech_style:{target_user_id:0,target_user_name:'',profile_text:'',sample_count:0,distilled_at_count:0}},skills:[{name:'memory_manage',description:'永久记忆',enabled:true,read_only:false},{name:'wiki_query',description:'Wiki',enabled:true,read_only:true}]};
    api.nativeMemories[id]??=[];
    const rest=suffix.slice('/native'.length);
    if(!rest&&method==='GET')data=api.nativeGroups[id];
    else if(!rest&&method==='PUT'){if(body.revision!==api.nativeGroups[id].settings.cg_config_revision)return reject(409,'group native changed');api.nativeGroups[id].settings={...api.nativeGroups[id].settings,...body.settings,cg_config_revision:api.nativeGroups[id].settings.cg_config_revision+1};data=api.nativeGroups[id];}
    else if(rest==='/memories'&&method==='GET')data={items:api.nativeMemories[id]};
    else if(rest==='/memories'&&method==='POST'){const item={id:api.nativeMemories[id].length+1,content:body.content,created_by:7,created_at:'2026-09-10T00:00:00Z'};api.nativeMemories[id].push(item);data={id:item.id,created:true,permanent:true};}
    else if(/^\/memories\/\d+$/.test(rest)&&method==='PUT'){const mid=Number(rest.split('/')[2]);const item=api.nativeMemories[id].find((m:any)=>m.id===mid);if(!item)return reject(404,'memory missing');item.content=body.content;data={id:mid,replaced:1,created:false};}
    else if(/^\/memories\/\d+$/.test(rest)&&method==='DELETE'){const mid=Number(rest.split('/')[2]);api.nativeMemories[id]=api.nativeMemories[id].filter((m:any)=>m.id!==mid);data={deleted:true};}
    else known=false;
   }
   else if(!suffix&&method==='GET')data=overview();
   ...
   else if(suffix==='/memories'&&method==='POST'){if(api.nativeEngine)return reject(409,'旧助手记忆已冻结只读；请使用原生永久记忆入口');const m={...memory(6,Number(id)),...body,version:1,source:{source_type:'admin_base',source_chat_id:Number(id),operator_id:7,operator_name:'Fixture 管理员',currently_verified:true,snippet:body.source_snippet},authority_level:'admin_base'};api.memories[id].push(m);data={memory:m};}
   ...
   else if(/^\/memories\/\d+$/.test(suffix)){if(api.nativeEngine&&method!=='GET')return reject(409,'旧助手记忆已冻结只读；请使用原生永久记忆入口');const m=api.memories[id].find(m=>m.id===Number(suffix.split('/')[2]));if(method==='PUT'){const {expected_version,...fields}=body;Object.assign(m,fields,{version:m.version+1,memory_type:'base',authority_level:'admin_explicit',source:{...m.source,source_type:'admin_explicit_correction',operator_id:7,operator_name:'Fixture 管理员',snippet:body.source_snippet}});}data={memory:m};}
   ...
   else if(path==='/api/admin/assistant/native'&&method==='GET')data=api.native;
   else if(path==='/api/admin/assistant/native'&&method==='PUT'){if(body.revision!==api.native.revision)return reject(409,'native config changed');api.native={...api.native,...body,revision:api.native.revision+1,engine:'native',automatic_fact_learning:false};data=api.native;}
   else if(/\/groups\/-\d+\/assistant\/native(?:\/|$)/.test(path)){
    const [,id,rest='']=path.match(/\/groups\/(-\d+)\/assistant\/native(.*)/)??[];
    api.nativeGroups[id]??={settings:{cg_config_revision:0,at_reply_mode:false,tts_mode:'off',mute_all_replies:false,scheduled_tasks:{cooldown_topic:{enabled:false,task_brief:''}},speech_style:{target_user_id:0,target_user_name:'',profile_text:'',sample_count:0,distilled_at_count:0}},skills:[{name:'memory_manage',description:'永久记忆',enabled:true,read_only:false},{name:'wiki_query',description:'Wiki',enabled:true,read_only:true}]};
    api.nativeMemories[id]??=[];
    if(!rest&&method==='GET')data=api.nativeGroups[id];
    else if(!rest&&method==='PUT'){if(body.revision!==api.nativeGroups[id].settings.cg_config_revision)return reject(409,'group native changed');api.nativeGroups[id].settings={...api.nativeGroups[id].settings,...body.settings,cg_config_revision:api.nativeGroups[id].settings.cg_config_revision+1};data=api.nativeGroups[id];}
...
export async function openAssistant(page:Page){await login(page);await page.context().addCookies([{name:'cg_csrf',value:'fixture-csrf',domain:'127.0.0.1',path:'/'}]);await page.goto('/groups/-1001/assistant');await expect(page.getByText(/实际状态|原生 SGB 助手/).first()).toBeVisible();await expect(page.getByLabel('切换群组')).toBeVisible();}
